...
diffSpeed = np.sum(np.absolute(testY[:,0] - rfpred[:,0]))/len(testY)
diffPressure = np.sum(np.absolute(testY[:,1] - rfpred[:,1]))/len(testY)
CCS, CCP = np.corrcoef(testY[:,0], rfpred[:,0])[0,1] , np.corrcoef(testY[:,1], rfpred[:,1])[0,1] 
MSES, MSEP = round(mean_squared_error(testY[:,0], rfpred[:,0]), 2), round(mean_squared_error(testY[:,1], rfpred[:,1]),2)
MAES, MAEP =  round(mean_absolute_error(testY[:,0], rfpred[:,0]), 2), round(mean_absolute_error(testY[:,1], rfpred[:,1]), 2)
